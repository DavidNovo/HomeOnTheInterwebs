---
title: "Fifteen Design"
date: 2020-07-30T12:47:38-05:00
draft: false
layout: static/css/fifteen.css
---

# Summary (h1 header) {#summary class="summary one"}

{{% div class="two" %}}
This is a new use of the CSS grid. Basically I had to make a shortcode for Hugo that would allow me to add the div tag where I needed it. Why? Because the div tag is super useful for layout and design.

Hugo currently uses Goldmark as its Markdown render so headers alredy have the option of adding class and IDs; other elements do not have this option.

Using div tags allowed me to seperate the headers and paragraphs and code-blocks into seperate grid columns.  Interesting effect, the headers are in one column, the paragraphs are in the second column, and the only code block is in the third column.  Could I have done the same thing in the CSS document, like this:
{{< highlight css>}}
  h1,
  h2,
  h3,
...
{{< / highlight >}}
Sure, this will work, but this means all headers will be in the first column of the CSS Grid.  Not a flexable solution.

Plus using a div tag allows me to keep paragraphs together in the same column and in the same cell.  See design fourteen as an example of same column and different rows.  

But then I could change the spacing bewteen the rows.

***

{{% /div %}}

# O penates includit sunt Graias contemptus rigidoque (h1 header) {#header-id-h1 class="title one"}
...
### Est te fit (h3 header class="subheader one") {class="one"}
...
## Senecta erat mihi quaque lumina (h2 header) {class="one"}
...
## Auditum visa (h2 header) {class="one"}
...
## Fugacibus aethere gloria (h2 header) {class="one"}
...
### Prunaque et teque. (h3 header) {class="one"}
...
#### Hippotades (h4 header) {class="one"}
