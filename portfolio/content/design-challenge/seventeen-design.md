---
title: "Seventeen Design"
date: 2020-08-01T19:25:01-05:00
draft: false
layout: static/css/seventeen.css
---
## Summary (h2 header) {#summary class="summary one"}

{{% div class="two" %}}
So now playing with the orientation of the text again.  Seeing about the using the first column as the header column where the headers are orientated vertically instead of horizontally.  The flow of the first column is top-down and left to right.  

The headers in the first column word-wrap if the content of the second column is small.  Note in this design, the headers in column one and the content in column 2 are in the same row.
